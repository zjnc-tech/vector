use super::{
    ...
    sink::{ClickhouseAnyTableSink, PartitionKey},
};
use crate::{
...
/// Configuration for the `clickhouse_any_table` sink.
#[configurable_component(sink(
    "clickhouse_any_table",
    "Deliver log data to a ClickHouse database any table."
))]
#[derive(Clone, Debug, Default)]
#[serde(deny_unknown_fields)]
pub struct ClickhouseAnyTableConfig {
...
impl_generate_config_from_default!(ClickhouseAnyTableConfig);

#[async_trait::async_trait]
#[typetag::serde(name = "clickhouse_any_table")]
impl SinkConfig for ClickhouseAnyTableConfig {
    async fn build(&self, cx: SinkContext) -> crate::Result<(VectorSink, Healthcheck)> {
        ...
        let sink = ClickhouseAnyTableSink::new(
            batch_settings,
            service,
            database,
            self.format,
